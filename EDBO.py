...
def parallel_grid_search(bounds, step_size):
    ...
    if filtered_results:
        best_result = min(filtered_results, key=lambda x: x["Fitness"])
...
step_size =  0.1 # Step size for the grid search
...
optimization_results = Parallel(n_jobs=num_cores)(
    delayed(edbo_optimization_run)(kp, kp_upper, ki, ki_upper, num_iterations=5)
